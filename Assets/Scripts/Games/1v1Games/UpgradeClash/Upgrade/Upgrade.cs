...
namespace UpgradeClash
{
    ...
    */
    public abstract class Upgrade
    {
        ...
        {
            float value = (float)unit.Amount / unit.Cap; // Base value is relative amount of units.
            if (IsInProgress)
            {
                ...
                progressRatio *= (1f / unit.Cap); // Adjust it to the max amount of units so the progress effect is always smaller than the unit amount
                value += progressRatio;
            }
            return value;
        }
